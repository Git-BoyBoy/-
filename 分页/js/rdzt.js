//jquery页面加载开始
$(function() {
		var pageDiv1 = new jsPage("rdzt_list", "all", "rdzt_fpage", "9");
...
jsPage.prototype.page = function() {
	if(this.totalpage <= 10) { //总页数小于十页  页码以十页为单位
		for(this.count = 1; this.count <= this.totalpage; this.count++) {
			if(this.count != this.cpage) {
				this.outstr = this.outstr + "<a href='javascript:void(0)' class='p' p='" + this.count + "' >" + this.count + "</a>";
			} else {
	...
	document.getElementById(this.page_obj_id).innerHTML = "<div><span class='info'> 第" + this.cpage + "页<\/span>" + this.outstr + "<span class='info'>共"+this.totalpage+"页<\/span>"+"<\/div>";
	this.outstr = "";
}